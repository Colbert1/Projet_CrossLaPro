...
int TCPServeur::createsocket() //Cette fonction va cr�er la socket
{
...
{
	SOCKADDR_IN sin = {0};
	const unsigned short port = 1050; //On met le serveur en �coute sur le port 1050
...
DWORD WINAPI TCPServeur::ClientThread(LPVOID params) //On cr�er un thread pour chaque client dans cette fonction
{
	char buf;
	...
	std::vector<SOCKET>::iterator it = std::find(connectedClients.begin(), connectedClients.end(), socket_to_send); //On regarde si celui ci est encore connect� ou non
	if (it != connectedClients.end())																				//Si il est d�connect� alors on ecrase sa socket
	{
